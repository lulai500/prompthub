...
/** 当月已执行次数（发起即计：in_progress/completed/failed；不含 pending） */
...
/** 从已加载的任务数组算当月用量（工作站/站主后台页面复用，免二次查询） */
export function countMonthlyUsageFromTasks(tasks: { status: string; created_at: string }[]): number {
  const start = monthStartIso();
  return tasks.filter(
    (t) =>
      ['in_progress', 'completed', 'failed'].includes(t.status) && t.created_at >= start
  ).length;
}
